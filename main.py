...
async def on_ready():
    await bot.tree.sync()
    print(f"{bot.user} est connecté.")@bot.tree.command(name="eveil", description="Éveille votre Nen")
...
if TOKEN is None:
    print("ERREUR : la variable TOKEN est introuvable.")
else:
    bot.run(TOKEN)
    )
